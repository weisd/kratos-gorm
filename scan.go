...
import (
	...
	ksql "github.com/go-kratos/kratos/pkg/database/sql"
	"gorm.io/gorm"
	"gorm.io/gorm/schema"
)
...
// Scan Scan
func Scan(rows *ksql.Rows, db *gorm.DB, initialized bool) {
		...
		case reflect.Slice, reflect.Array:
			...
			if Schema != nil {
				if reflectValueType != Schema.ModelType && reflectValueType.Kind() == reflect.Struct {
					Schema, _ = schema.Parse(db.Statement.Dest, _cacheStore, db.NamingStrategy)
				}
